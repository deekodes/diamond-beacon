...
library LibDiamondBeacon {
    // 32 bytes keccak hash of a string to use as a diamond storage location.
    bytes32 constant DIAMOND_BEACON_STORAGE_POSITION = keccak256("diamond.standard.beacon.storage");
    ...
    function diamondBeaconStorage() internal pure returns (DiamondBeaconStorage storage ds) {
        bytes32 position = DIAMOND_BEACON_STORAGE_POSITION;
    ...
    function contractOwner() internal view returns (address contractOwner_) {
        contractOwner_ = diamondBeaconStorage().contractOwner;
    }

    function enforceIsContractOwner() internal view {
        require(msg.sender == diamondBeaconStorage().contractOwner, "LibDiamondBeacon: Must be contract owner");
    }
    ...
    function addFacet(DiamondBeaconStorage storage ds, address _facetAddress) internal {
